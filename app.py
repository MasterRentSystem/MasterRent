...
# --- FUNZIONE GENERAZIONE XML (CORRETTA) ---
def genera_xml_fattura(c):
    ...
    # PROTEZIONE ERRORI: Controllo sicuro sulla nazionalità
...
def genera_fattura_pdf(c):
    ...
    pdf.set_font("Arial", "", 14); pdf.cell(0, 10, safe(f"FATTURA N. {c.get('numero_fattura', '0')}"), ln=True, align="C")
    pdf.ln(10); p = float(c.get('prezzo', 0)); imp = p/1.22; iva = p-imp
    pdf.set_font("Arial", "", 10)
    pdf.cell(110, 10, safe(f"Noleggio {c.get('modello', '')} {c.get('targa', '')}"), border=1)
    ...
    try:
        ...
        for r in res.data:
            with st.expander(f"Fattura {r['numero_fattura']} - {r['cognome']} ({r['targa']})"):
                ...
                col_a.download_button("📜 PDF Cliente", genera_fattura_pdf(r), f"Fattura_{r['numero_fattura']}.pdf")
                
                # Ora questo tasto non darà più errore anche se i dati sono incompleti
