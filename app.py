...
def genera_contratto_legale(c):
    ...
    pdf.ln(5); pdf.set_font("Arial", "B", 10); pdf.cell(0, 7, " ANAGRAFICA CLIENTE", ln=True, border=1)
    ...
    pdf.cell(190, 7, safe(f"Indirizzo: {c.get('indirizzo_cliente','')}"), border="LRB", ln=True)
    
    pdf.ln(3); pdf.set_font("Arial", "B", 10); pdf.cell(0, 7, " DATI NOLEGGIO", ln=True, border=1)
...
with t1:
    with st.form("form_secure"):
        ...
        st.subheader("📹 Stato del Veicolo")
        video_mezzo = st.file_uploader("Registra o carica video danni", type=["mp4", "mov"])
        
        f1 = st.file_uploader("Fronte Patente")
        canvas = st_canvas(height=150, width=400, stroke_width=3, key="sig_secure")
        ...
        if st.form_submit_button("REGISTRA"):
            try:
                ...
                dati = {
                    ...
                    "url_fronte": u_f, "url_video": u_v, "pec": pec, "codice_univoco": sdi,
                    "numero_patente": pat, "nazionalita": naz, "telefono": tel, "inizio": str(datetime.now().date())
                }
    ...
    for r in res.data:
        if search.lower() in f"{s(r['cognome'])} {s(r['targa'])}".lower():
            with st.expander(f"N. {r['numero_fattura']} - {r['cognome']}"):
                if r.get('url_video'): st.video(r['url_video'])
                st.download_button("📜 Contratto", genera_contratto_legale(r), f"C_{r['id']}.pdf")
