...
if lista_aziende:
    ...
    if menu == "📝 Nuovo Contratto":
        ...
        st.subheader("✍️ Firma per Contratto e Responsabilità Multe")
            ...
            if cliente and targa and canvas.image_data is not None:
                try:
                    ...
                    dat = {
                        ...
                        "tipo_doc": tipo_doc, "num_doc": num_doc, "scadenza_patente": str(scad_p), 
                        "telefono": telefono, "targa": targa, "km_uscita": km_uscita, "prezzo_tot": str(prezzo_tot), 
                        "azienda_id": azienda['id'], "data_inizio": str(datetime.date.today()), "note": note_extra
                    }
                    res = supabase.table("contratti").insert(dat).execute()
                    id_row = res.data[0]['id']

                    # GENERAZIONE PDF (MODULO MULTE + CONTRATTO)
                    pdf = fpdf.FPDF()
                    pdf.add_page()
                    pdf.set_font("Arial", 'B', 16); pdf.cell(0, 10, txt=clean_t(azienda['nome_azienda']), ln=1, align='C')
                    pdf.ln(5)

                    # --- SEZIONE RESPONSABILITA' MULTE ---
                    pdf.set_fill_color(240, 240, 240)
                    pdf.set_font("Arial", 'B', 11); pdf.cell(0, 8, " DICHIARAZIONE RESPONSABILITA' INFRAZIONI (Art. 196 CdS)", ln=1, fill=True)
                    pdf.set_font("Arial", size=9)
                    testo_multe = (f"Il sottoscritto {cliente}, nato a {luogo_nascita}, CF: {cf}, residente in {residenza}, "
                                   f"DICHIARA di avere la piena disponibilita del veicolo targato {targa} e si assume "
                                   f"la totale responsabilita per ogni infrazione al Codice della Strada commessa durante il noleggio.")
                    ...
                    pdf.set_font("Arial", 'I', 7); pdf.cell(0, 5, "Firma del Conducente (Assunzione Multe)", ln=1)
                    ...
                    # TASTI AZIONE
                    c1, c2 = st.columns(2)
                    txt_wa = f"MASTERRENT MULTE\nVeicolo: {targa}\nCliente: {cliente}\nCF: {cf}\nDoc: {num_doc}\nIl cliente ha firmato l'assunzione di responsabilità."
                    c1.markdown(f'''<a href="https://wa.me/{telefono}?text={urllib.parse.quote(txt_wa)}" target="_blank"><button style="background-color:#25D366;color:white;width:100%;border:none;padding:12px;border-radius:5px;font-weight:bold;cursor:pointer;">📲 Dati Multe (WA)</button></a>''', unsafe_allow_html=True)
                    c2.download_button("📥 Scarica PDF Modulo Multe", pdf_bytes, file_name=f"Modulo_Multe_{targa}.pdf")
        ...
        if res.data:
            ...
            st.link_button("📄 Vedi Modulo Multe", f"{url}/storage/v1/object/public/contratti_media/doc_{c['id']}.pdf")
            st.link_button("🪪 Vedi Patente", f"{url}/storage/v1/object/public/contratti_media/pat_{c['id']}.jpg")
